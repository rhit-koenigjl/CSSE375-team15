...
public class GameComponent extends JComponent {
	...
	public void generateLevel(String inputFilename) {
		tiles.clear();
		enemies.clear();
		ArrayList<int[]> hunterSeekersToAdd = new ArrayList<int[]>(); // HunterSeekers to be added after hero added
		...
		for(int y = 0;y < l.size();y ++) { //reads line number
			for(int x = 0;x < l.get(y).length();x ++) { //reads line length
				...
				case 'S':
					...
					hunterSeekersToAdd.add(addedHunterSeeker); //adds HunterSeeker info to a ArrayList to be created after everything else
					break;
				case 'B':
		...
		for(int[] HSInfo : hunterSeekersToAdd) { 
			...
			//Therefore, the HunterSeekers are added after everything else to garuntee the hero has been created beforehand
	...
	public void paintComponent(Graphics g) {
		...
		case "game":
			...
			g2.drawString("Lives: "+hero.getNumberOfLives()+"      Level: "+levelIndex + "         Score: " + score, 25, 30);
